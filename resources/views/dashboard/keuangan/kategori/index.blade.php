...
@endphp
    ...
    @if ($role == 'umkm')
        <div x-data="{ openNotification: true  }" x-transition.opacity x-cloak x-show="openNotification" class="bg-warning p-4 rounded-md mb-4 flex gap-4 justify-between items-center">
            <p class="text-sm md:text-base">
                Periksa kembali laporan keuangan bulan ini. Ada beberapa transaksi yang perlu diverifikasi.
            </p>
            <button 
    ...
    <div x-data="{
            ...
            modalTitle: '{{ old('_method') === 'PUT' ? 'Edit Kategori Keuangan' : 'Tambah Kategori Keuangan' }}',
            categoryId: {{ session('edit_category_id') ?? 'null' }},
            categoryName: '{{ old('name', '') }}',
            categoryTypeId: '{{ old('type_id', '') }}',
            errors: {},
            editFormAction() {
                    ...
                    >
                        ...
                        >
                            <div class="flex justify-between items-center">
                                <h2 class="text-xl font-semibold">Filter Kategori Keuangan</h2>
                                <button 
                                ...
                                <div class="my-6 space-y-2">
                                    <div>
                                        ...
                                        <x-input.select 
                                            ...
                                            :options="[
                                                '' => 'Semua Tipe',
                                                ...$transactionTypes->pluck('name', 'id')->toArray()
                                            ]"
                ...
                <table class="w-full table-auto text-sm">
                    <thead class="bg-secondary text-light">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left">Tipe Keuangan</th>
                    ...
                    <tbody class="text-gray-700">
                        @forelse ($categories as $index => $category)
                            <tr class="border-b border-b-gray-300 even:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    <x-badge class="w-max max-w-full">{{ $category->type->name ?? '-' }}</x-badge>
                                </td>
                                <td class="px-4 py-2">{{ $category->transactions_count ?? 0 }}</td>
                                <td class="px-4 py-2">
        ...
        >
            ...
            >
                ...
                <form action="" method="POST" id="form" class="mt-6">
                    ...
                    <div class="mb-4">
                        <x-label :isRequired="true" for="type_id">Tipe Keuangan</x-label>
                        <x-input.select 
                            name="type_id"
                            :options="$transactionTypes->pluck('name', 'id')->toArray()"
                            x-model="categoryTypeId"
                            class="{{ $errors->has('type_id') ? 'border-danger focus:border-danger' : '' }}"
                        />
                        @error('type_id')
