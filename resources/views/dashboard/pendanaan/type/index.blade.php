...
@section('content')
    ...
    <div x-data="{
            ...
            modalTitle: '{{ old('_method') === 'PUT' ? 'Edit Tipe Pendanaan' : 'Tambah Tipe Pendanaan' }}',
            ...
            editFormAction() {
                ...
                if (this.editMode && this.typeId) {
                    form.action = '{{ route('admin.dashboard.pendanaan.type') }}/' + this.typeId;
            ...
            deleteFormAction() {
                ...
                formDelete.action = '{{ route('admin.dashboard.pendanaan.type') }}/' + this.typeId;
            },
            resetForm() {
                this.typeName = '';
                this.errors = {};
        ...
        <div class="flex flex-col md:flex-row gap-4 justify-between items-center">
            <div class="flex gap-2 items-center w-full md:w-auto">
                <form method="GET" action="{{ route('admin.dashboard.pendanaan.type') }}" class="flex w-full">
                    <x-form.search 
                        name="search"
        ...
        <div class="mt-4">
            <div class="overflow-x-auto bg-light shadow-xl rounded-lg">
                ...
                @if($fundingTypes->hasPages())
                    <div class="flex justify-between items-center bg-secondary text-light px-4 py-2">
                        <p class="text-sm">
                            Showing {{ $fundingTypes->firstItem() }} to {{ $fundingTypes->lastItem() }} of {{ $fundingTypes->total() }} results
                        </p>
                        <div class="flex rounded-lg overflow-hidden">
                            @if($fundingTypes->onFirstPage())
                                <span class="bg-gray-600 px-4 py-2 cursor-not-allowed opacity-50">&lt;</span>
                            @else
                                <a href="{{ $fundingTypes->previousPageUrl() }}" class="bg-gray-800 px-4 py-2 hover:bg-gray-700">&lt;</a>
                            @endif

                            @foreach($fundingTypes->getUrlRange(1, $fundingTypes->lastPage()) as $page => $url)
                                @if($page == $fundingTypes->currentPage())
                            ...
                            @if($fundingTypes->hasMorePages())
                                <a href="{{ $fundingTypes->nextPageUrl() }}" class="bg-gray-800 px-4 py-2 hover:bg-gray-700">&gt;</a>
                            @else
                                <span class="bg-gray-600 px-4 py-2 cursor-not-allowed opacity-50">&gt;</span>
                            @endif
                        </div>
