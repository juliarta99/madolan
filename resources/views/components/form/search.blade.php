...
<form {{ $attributes->merge(['class' => 'flex justify-start items-center']) }}>
    ...
    <button 
        type="submit" 
        class="rounded-tr-md rounded-br-md hover:bg-secondary transition-all duration-200 bg-primary md:py-1.5 lg:py-2 px-2 py-1 border border-primary cursor-pointer {{ $styleButton }}"
        {{ $attributes->only(['wire:click', 'wire:submit.prevent']) }}
    >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="size-6 fill-light {{ $styleIcon }}">
